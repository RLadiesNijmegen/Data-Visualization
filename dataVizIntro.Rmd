...
- Bar plots / stacked bar plots) are OK for proportion data
    ...
    - Simple plot is fast and easy to percieve
...
##Ok Barplots
Here's a subtle improvement: tabulate the data so that we plot the proportion of individuals with improved sleep quality by group.  If that's the actual question we're intersted in, this might be the right plot.  Why:  The data all exist between 0 and 1, which are the limits on the scale. 
```{r okbar, echo=T}
 ...
 * Portray variability when you can, but also tell a clear story
    ...
    + adding points is a greatr way to show variability, but sometimes can be overwheliming
